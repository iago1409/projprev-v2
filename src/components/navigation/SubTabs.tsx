...
interface SubTabsProps {
  activeTab: 'contrato' | 'complementares' | 'vinculo' | 'sucessao' | 'termino' | 'vinculos-incorporados' | 'consolidacao';
  cpf?: string;
}
...
export const SubTabs: React.FC<SubTabsProps> = ({ activeTab, cpf }) => {
  ...
  const handleTabClick = (tab: string) => {
    ...
    } else if (tab === 'vinculos-incorporados') {
  ...
  return (
    <div className="border-b border-gray-200">
      <nav className="-mb-px flex space-x-8">
        ...
        <button 
          onClick={() => handleTabClick('vinculos-incorporados')}
          className={`border-b-2 py-2 px-1 text-sm font-medium transition-colors ${
            activeTab === 'vinculos-incorporados'
              ? 'border-blue-500 text-blue-600'
              : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
          }`}
        >
